...
function App() {
  const [userTodos, setUserTodos] = useState([]) //Akobo
  ...
  const handleStoringOfData = (todo) => {
...
const handleStatus = (id) => {
    setUserTodos(userTodos.map(todo => todo.id === id ? {...todo, status: !todo.status} : todo))
}

const updateTodo = (editted) => {
  setUserTodos(userTodos.map(todo => todo.id === editted.id ? editted : todo))
}
...
console.log(userTodos)
  return (
    <>
      ...
      <TodoForm handleStoringOfData={handleStoringOfData} />
      <TodoList userTodos={userTodos} handleStatus={handleStatus} handleDelete={handleDelete} updateTodo={updateTodo}/>
